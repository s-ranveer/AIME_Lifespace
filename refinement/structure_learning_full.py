...
from pgmpy.estimators import  BIC, MaximumLikelihoodEstimator, BayesianEstimator, HillClimbSearch
...
def sample_from_bn(M: DiscreteBayesianNetwork, n: int):
    return M.simulate(n_samples=n)


def sample_edges(M: DiscreteBayesianNetwork, n_edges: int):
    V = list(M.nodes)
    E = list(M.edges)
...
def refine_bn(
    M0: DiscreteBayesianNetwork, data: pd.DataFrame, state_names: Dict[str, List], **kwargs
):
    """
    Parameters:
      M0: Initial BN
...
class StructuredBicScore(BIC):
    ...
    def local_score(self, variable, parents, return_cpd=False):
        ...
        est = BayesianEstimator(model, self.data, state_names=self.state_names)
        table_cpd = est.estimate_cpd(variable, equivalent_sample_size=0.1)
