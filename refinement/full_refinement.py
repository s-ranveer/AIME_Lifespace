...
results_path = os.path.join(tld, "refinement", "results", "intersection")
initial_edges_path = os.path.join(tld, "graphs", "intersection")
blacklist_path = os.path.join(tld, "refinement", "blacklists.csv")
"""
...
if __name__ == "__main__":
    ...
    for llm_edges_file in os.listdir(initial_edges_path):
        if llm_edges_file.endswith(".graph"):
            ...
            black_list_df = pd.read_csv(blacklist_path)
            # We are not doing bootstrap but processing the original data
            ...
            # Remove the edges which are in the blacklist
            edges = [e for e in edges if e not in black_list]
            ...
            M1 = refine_bn(M0.copy(), data, state_names, scoring_method=BIC(data, state_names=state_names),
                           tabu_length=0, black_list=black_list, subtractive_refinement_only=False)
